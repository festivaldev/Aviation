...
<%
	try {
		...
		if (request.getParameter("query") != null && !request.getParameter("query").isEmpty()) {
			PreparedStatement statement = conn.prepareStatement("(SELECT * FROM `airports` WHERE `iata_code` = ?) UNION (SELECT * FROM `airports` WHERE `name` LIKE ? OR `municipality` LIKE ? ORDER BY `name` ASC LIMIT 5)");
